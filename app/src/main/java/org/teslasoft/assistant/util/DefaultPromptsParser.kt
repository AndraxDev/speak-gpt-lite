...
class DefaultPromptsParser {
    ...
    fun parse(type: String, text: String, context: Context) {
        val lng = Preferences.getPreferences(context, "").getLanguage()
        when (type) {
            "explanationPrompt" -> {
                if (lng in languagesSupported) {
                    listener!!.onCompleted(String.format(explanationPrompt.getValue(lng), text))
                } else {
            ...
            "summarizationPrompt" -> {
                if (lng in languagesSupported) {
                    listener!!.onCompleted(String.format(summarizationPrompt.getValue(lng), text))
                } else {
                    listener!!.onCompleted(String.format(summarizationPrompt.getValue("en"), text))
                }
            }
            else -> {
